# LSCC CellChat analysis of high-CNV malignant Clusters 0 and 8
# Evaluates outgoing signaling to selected tumor-microenvironment populations.
...
# Leave as NA unless automatic detection cannot locate the prerequisite files.
...
# FALSE: reuse an existing compatible CellChat RDS when available.
# TRUE : recompute CellChat from the saved Seurat objects.
...
# Step 2: standard manuscript figure settings
...
# Figure 13 contains two network/bubble rows (Clusters 0 and 8).
# The height is reduced to keep the final figure compact while retaining readability.
...
# Heatmap typography retained for consistent manuscript-wide settings
HEATMAP_FONT_PT <- 8
...
# Radial-network node colours. The source is a muted blue; target populations
# use distinct soft pastels so they remain easy to identify in the fan layout.
...
# Single muted edge colour used in all radial outgoing network panels.
...
make_network_layout <- function(cluster_id, available_targets) {
  # Fan/radial layout: the malignant source cluster is positioned below,
  # and target populations are arranged around an upper semicircle.
...
# Step 5: detect a completed CellChat object or build a new one
...
# Step 6: validate CellChat content and export communication tables
...
# Step 7: prepare network and top-five ligand–receptor data
...
# Shared scales make probability, P-value-derived dot sizes, and edge widths
# comparable between the three source clusters.
...
# Step 8: manuscript-style plotting functions
...
# Step 9: build and export figure 13
...
# The network column is widened by ~16% to improve readability of nodes, edges,
# and labels, while retaining sufficient width for the ligand–receptor bubble plots.
...
save_plot_all_formats(
  ...
  filename_stem = "Figure_13_CellChat_clusters_0_8",
  dir_path = OUTPUT_DIR,
...
final_png <- file.path(
  OUTPUT_DIR,
  "Figure_13_CellChat_clusters_0_8.png"
)
final_tiff <- file.path(
  OUTPUT_DIR,
  "Figure_13_CellChat_clusters_0_8.tiff"
)
final_pdf <- file.path(
  OUTPUT_DIR,
  "Figure_13_CellChat_clusters_0_8.pdf"
)
...
summary_lines <- c(
  ...
  "Each source row contains an outgoing network and selected ligand–receptor pairs.",
  "Probability, point-size, and edge-width scales are shared across clusters.",
...
writeLines(
  summary_lines,
  file.path(OUTPUT_DIR, "Figure_13_CellChat_clusters_0_8_summary.txt")
)

writeLines(
  capture.output(sessionInfo()),
  file.path(OUTPUT_DIR, "Figure_13_CellChat_clusters_0_8_sessionInfo.txt")
)
